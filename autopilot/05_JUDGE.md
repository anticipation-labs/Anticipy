...
3. Tamper scan. Look at the builder's last commit diff. Auto-FAIL and HALT on any edit under `tests/`, `judge/`, or `realdays/holdout/`; any edit to `scripts/realday.sh` except narrow, generic harness plumbing for cached transcripts or explicit clock metadata; any hardcoded literal tied to the eval in product code, prompts, or tests outside policy text (the owner's name, their apps, or student-specific words such as FAFSA or roommate); any milestone marked done on fewer than its required real checks; or any pass claimed on self-made or synthetic data. Also scan for secrets and raw held-out or transcript files. Write `TAMPER` in the verdict when this fires.
4. For the current clean M0, use a judge-owned typed, fully time-grounded, safe, reversible instruction. Do not use held-out audio for clean M0. For later realday/audio milestones, take a real day the builder has never seen from `realdays/holdout/`. The builder must never read `realdays/holdout/`. A held-out day is burned and rotated out only when it contributes to a milestone PASS. A FAILED lap does not burn a day; since the builder never reads held-out content, the judge may reuse those days on later laps. A mere inventory list of available holdout filenames does not burn those days.
5. Run the whole system on the typed clean instruction for M0, or on the held-out day for later audio milestones.
