...
### Deployment evidence (2026-09-15, run by Claude on the owner's instruction)

Both PRs squash-merged into `cloudflare-backend`: [#67](https://github.com/anticipation-labs/Anticipy/pull/67)
(code, `0c22445a`) and [#66](https://github.com/anticipation-labs/Anticipy/pull/66)
(docs, `2a3fdf17`, the deployed tip). The two pushes fired build-only iOS runs
([34976788951](https://github.com/anticipation-labs/Anticipy/actions/runs/34976788951),
[34976855674](https://github.com/anticipation-labs/Anticipy/actions/runs/34976855674)), both green.

| Step | Run | Result | Verified live |
| --- | --- | --- | --- |
| API + extension download | [brain-deploy 34979595656](https://github.com/anticipation-labs/Anticipy/actions/runs/34979595656) `component=api` | success | GET `/api/health` → `x-anticipy-revision: 2a3fdf1702a7492321355a27fa7b1f97243c19ac`; all three download aliases hash to `a6a8259d95b5d352a9354795210921e7835e151ac83a888e209b4b5cf0652f07` (0.18.3). Before the deploy they served `3e9c6e22…` (0.18.2). |
| iPhone build 177 | [ios-testflight 34980473198](https://github.com/anticipation-labs/Anticipy/actions/runs/34980473198) | success | `Source build 177 is above every build Apple holds; uploading as 177`; `UPLOAD SUCCEEDED with no errors`; `build 177 is VALID`; the private pilot invite step ran (owner-authorised 2026-09-14). |
| Existing-tester audience | [asc-query 34987072438](https://github.com/anticipation-labs/Anticipy/actions/runs/34987072438) `ASSIGN_EXISTING build=177` | assignment `covered` (write attempted, 6/6 existing testers covered, 3 linked directly, 0 missing); the final audience gate exited 1 | `who-can-install`: Internal (2, both INSTALLED) and the private pilot (1, INSTALLED) linked; tester states 4 INSTALLED / 2 INVITED. The red is the two invitations still unaccepted — the same condition the 13 September snapshot records. Only those two people can clear it. |
| Brain (cap 100) | [brain-deploy 34980570057](https://github.com/anticipation-labs/Anticipy/actions/runs/34980570057) `component=brain` | success | preflight passed; after the immediate rollout restarted owner workers (~8 min of `metadata_request_failed`), the fleet check returned `ready: true, revision_matches: true, runtime_source_verified: true, capacity 100, covered_owners 10`; `historical_uncertain_effects: 3` and `invalid_metadata: 5` are the pre-existing residues this board already carries. |

Rung for every code line above is therefore `deployed, unproven`. What moves
any of them to `live-proven` is unchanged: the owner's own journey on the live
download and build 177 — pair, errand, restart, release from the phone, "Not
linked", same-code re-pair, errand again — and, for the brain, a 202/503 shape
observed through the metadata projection that is still to be written.
