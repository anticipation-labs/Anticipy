...
//
// Ported out of spike/two-hands/src/connections/words.ts, which had been
// through two adversarial rounds. The behaviour and the reasoning are the
// spike's; what changed on the way into the Worker is at the bottom of this
// header.
//
...
//   1. `REAL_MOMENTS` is a local table typed `Record<NudgeTrigger, number>`
//      instead of a runtime import of the contract's `TRIGGER_SCORE`, so the
//      deployed Worker carries no runtime edge into the spike tree. It is not
//      free-floating: the type makes a missing or invented trigger a COMPILE
//      error, and test/connections-provider.test.ts reads the contract's own
//      source and goes red if the two tables stop agreeing. A sixth trigger
//      cannot silently fail to be a real moment here.
//   2. `URL_LIKE` sees SCHEMELESS urls. This is the fix named in the port
//      brief: `https?://` alone was blind to `connect.<vendor>.dev/link/abc`,
//      which every phone linkifies on sight, so "exactly one URL and it must be
//      ours" could be satisfied by a message carrying two tappable links, one
//      of them the vendor's and already dead.
//
...
// the deployed Worker has no dependency on the spike tree — but the types are
// the contract's own declarations rather than a second copy of them. See the
// same block in provider.ts.
import type {
  NudgeTrigger,
  PermissionWords,
  ToolkitMeta,
 ...
 *
 *  The table is typed `Record<NudgeTrigger, number>` against the contract's own
 *  union, so a sixth trigger added there is a COMPILE error here rather than a
 *  silent hole — the spike derived the same set from `TRIGGER_SCORE` at run
 *  time, and this keeps that guarantee without giving the deployed Worker a
 *  runtime edge into the spike tree. The numbers are the contract's right-time
 *  scores, kept beside the names so the two cannot be edited apart; nothing in
 *  this file reads them, and test/connections-provider.test.ts pins both halves
...
// time — by the Worker's bundler and by `node --experimental-strip-types`
// alike — so the declared return type of an injected function is a comment.
// Every reply is checked as if it came off a wire, because it did.
...
export async function askText(
  ...
  if (urls.length !== 1) {
    ...
    // dead one. A SCHEMELESS one counts: a phone linkifies `connect.x.dev/ab`
    // exactly as it linkifies `https://connect.x.dev/ab`, and the person taps
    // whichever is nearer their thumb.
    return refuse(
      "extra-link",
