...
function commandPrompt(said: string, commands: readonly TextCommand[], context?: TextCommandContext): ChatMessage[] {
  const system = [
    ...
    "",
    "Allowed values for the command FIELD inside the command object:",
    ...commands.map((c) => `- ${c}`),
    "An operation name is never a kind. For any identified operation, keep",
    '"kind":"command" and put the operation in the separate "command" field.',
    "",
