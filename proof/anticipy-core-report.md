# Anticipy: memory, orchestration, voice — proof report
...
- `brain/memory.py` — temporal knowledge graph on SQLite (not a JSON file, not RAG):
  ...
  Commitments carry an open/done/cancelled lifecycle — Anticipy's to-do list.
- `brain/anticipy.py` — Anticipy, the single responsible orchestrator + personality:
...
- iOS: Anticipy briefing card on the home feed; onboarding introduces her by name.
...
| Anticipy orchestration offline | PASSED — `proof/test_anticipy.py` (4/4) |
| Live spine vs real PocketBase | PASSED — `proof/test_anticipy_live.py`: commitment → job held `awaiting_confirm` → in-app confirm PATCH released it → agent done → Anticipy closed the loop and resolved the memory commitment |
...
proof/test_anticipy.py    PASS x4
proof/test_anticipy_live.py
  ...
  Briefing: "How goes it today? I'm Anticipy. I overheard 1 thing worth
