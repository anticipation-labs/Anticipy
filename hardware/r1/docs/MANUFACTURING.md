...
## Enclosure (aluminum unibody, PLAUD-style — current architecture)

Generated by `enclosure/gen_alu_body.py`. Every customer-touch surface is
aluminum; parts clip together with an internal slide-and-lock clasp (no glue,
no screws, no 3D-printed parts):

- `alu_body.step` — CNC 6061 unibody tub (sides + rear in one piece),
  bead blasted + natural matte anodized. Quote at JLCCNC.
- `alu_face_front_v3.step` — CNC 6061 top plate with mic ports (2x 1.2 mm),
  button hole (4.2 mm), LED window (1.6 mm), antenna opening, and 4 underside
  L-hooks. Drop into the rebate, slide 1.5 mm toward the USB end to clasp the
  hooks under machined internal lips, then press the POM lock keys
  (`pom_lock_key.step`) into the notch gaps to block back-slide.
- `pc_antenna_window.step` — machined polycarbonate flanged insert seated in
  a floor recess over the MDBT50Q antenna keepout, captured by the
  battery/PCB stack (radio-transparent; the only non-aluminum exterior part).
- The undercut clasp channels require a T-slot/undercut cutter — confirm with
  the CNC vendor before ordering; exterior skin at the channels is >= 0.8 mm.

Legacy (superseded): the printed PC center + flat 5052 faces flow
(`gen_enclosure.py`, `pc_center.3mf`) is kept only for bench fit-checks.
