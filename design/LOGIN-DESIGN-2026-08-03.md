...
**What it does not solve.** Six things, worth saying plainly so nobody mistakes this plan for more than it is. One of the six has since been closed and is marked so rather than deleted, because the reason it was ever open is the useful part:
...
- **The inbound SMS webhook signature check** — CLOSED since this was written. `sms.pb.js:6-26` now validates `X-Twilio-Signature` against `ANTICIPY_TWILIO_WEBHOOK_URL` with the dependency-free HMAC-SHA1 signer in `pb_hooks/twilio_signature.js` (constant-time compare, cross-checked against Twilio's published vector) and answers a mismatch with 403. What this bullet originally said was that the token rode in the URL query string, landing in Railway logs, Twilio's console and every proxy in between, and that the only code in the repo doing it properly was `backend/sms_server.py:77`. That file was a second Python copy of the whole endpoint which nothing ever deployed: `backend/Dockerfile` COPYs only `pb_migrations`, `pb_public` and `pb_hooks`, and `start.sh` execs only `./pocketbase serve`, so no image ever installed Python to run it. It was deleted 2026-08-19.
