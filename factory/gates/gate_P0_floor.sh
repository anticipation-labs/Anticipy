...
# Always use a private scratch id — NEVER write into the calling lap's dir.
LAP="floorcheck-$(date -u +%H%M%S)"
TMP="logs/factory/runs/$LAP"
...
"$PY" factory/bin/persona_run.py --bank factory/personas/dev \
  --personas parent_dana,student_kayla --lap "$LAP" --tier stub \
...
SCRATCH="logs/factory/laps/$LAP"
mkdir -p "$SCRATCH"
cp "$TMP/metrics.json" "$SCRATCH/metrics.json"
...
print(m.cost_from('$SCRATCH/canned.json'))")
