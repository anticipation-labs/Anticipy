# Chrome Web Store — Anticipy Codex Version listing package
...
## Name
Anticipy Codex Version — your hands on the computer

## Summary (132 chars max)
Anticipy Codex Version does the task you already approved, in your own logged-in Chrome —
and shows you every page she touches.

## Description
Anticipy Codex Version is a personal assistant that listens to your day, remembers what
matters, and handles the follow-through. This extension is her hands: once you
approve a task in the Anticipy Codex Version app or by text, she opens the pages, fills the
...
- It never types your password or your card number, hands the task back at
  ...
  all.
- Everything happens in a tab group called "Anticipy Codex Version" that you can open and
  watch, or take over.
- The text of the page Anticipy Codex Version is working on, and a picture of that tab, are
  sent to an AI model so it can decide the next click — along with the name,
  email and phone you saved in Anticipy Codex Version, when a form asks for them. Nothing
  else is collected, and nothing is sold or shared.

Requires the Anticipy Codex Version iPhone app and an Anticipy Codex Version account.
...
- **"a tab group called Anticipy Codex Version"** — `chrome.tabGroups.update(group, { title:
  "Anticipy Codex Version", color: "yellow", collapsed: true })`, in `agent_loop.js` for
...
- **"nothing else is collected"** — the only hosts the extension ever contacts
  are the Anticipy Codex Version backend (`DEFAULT_BASE` in `background.js`), OpenRouter,
...
- **"requires the Anticipy Codex Version iPhone app"** — the phone claims the pair code
...
- **`debugger`**: Anticipy Codex Version performs trusted user-gesture clicks and typing on
  ...
  (`userCancelledTabs`).
- **`tabs` / `tabGroups`**: the work happens in a labelled "Anticipy Codex Version" tab group
...
- **What leaves the browser, and where it goes.** For each step of a task: the
  ...
  goals, statuses and results are written to the user's own Anticipy Codex Version backend
...
Anticipy Codex Version normally pairs to an iPhone app, and I know you don't have one. You
...
backend also pushes new jobs over SSE — a collapsed tab group named "Anticipy Codex Version"
appears. Expand it to watch. Chrome will show its yellow *"Anticipy Codex Version started
...
- [x] **Rebuild `backend/pb_public/anticipy-codex-version-extension.zip` from the final
...
1. **What is sent off the machine, and to whom.** The page text and element
   ...
   phone, date of birth, and any facts the user told Anticipy Codex Version — when a form
   asks for them. Task goals, statuses and results, to the user's own Anticipy Codex Version
...
- Screenshots: 1280×800, 3–5 — the popup, a task running in an expanded
  "Anticipy Codex Version" tab group, and the pairing page.
