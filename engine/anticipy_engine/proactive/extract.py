...
WHAT IS NOT A TASK — a QUESTION, or an AMBIENT REPORT of someone else's state/need (NEVER list it, even though it names an action): (a) a question about whether SOMEONE ELSE did/will do something is a CHECK — "Did you grab the dry cleaning?", "Have you emailed Sarah yet?" -> drop. (b) a report that a THIRD PARTY has a need, or that something is in some STATE, with NO request or instruction aimed at the speaker, is ambient context, NOT the speaker's task — "Casey told me grandma needs her prescription", "the dishwasher is leaking", "the vendor wants the contract back", "Sarah said she'd handle the catering", "Mom keeps reminding me to call grandma" -> drop, list nothing. The line is the difference between an IMPERATIVE/REQUEST aimed at the speaker ("can you pick up Leila", "pick up the kids") — a task — and a STATEMENT OF A NEED OR STATE ("grandma needs her prescription", "the heater is broken") — not a task. When unsure, prefer DROP: surfacing ambient context as a task is a nag.
