...
def _gmail_compose_screenshot(out_png: Path, trajectory_dir: str = "",
    ...
    #
    # Prefer the `websockets` package over `websocket-client`. The latter
    # always sends an `Origin: http://<host>:<port>` header which Chrome
    # rejects with 403 when launched with the engine-default flag
    # `--remote-allow-origins=http://localhost:*` (Chrome 148 enforces a
    # literal match, not a glob). The frozen action engine uses
    # `websockets.sync.client.connect` for the same reason.
    try:
        import base64
    ...
    try:
        ...
        return info
    def _is_compose_tab(t: dict) -> bool:
        if t.get("type") != "page":
            return False
        url = t.get("url") or ""
        title = t.get("title") or ""
        if "mail.google.com" not in url:
            return False
        # Match all known Gmail compose URL/title shapes:
        #   - ?compose=<id> (classic deep link)
        #   - ?fs=1&tf=cm   (full-screen new compose; what current Gmail
        #     opens for a fresh draft via the +compose JS path)
        #   - title starts with "Compose Mail - " (tab title is the most
        #     reliable signal once Gmail finishes rendering)
        if "compose=" in url:
            return True
        if "tf=cm" in url:
            return True
        if title.startswith("Compose Mail"):
            return True
        return False

    compose = next((t for t in tabs if _is_compose_tab(t)), None)
    if not compose:
...
def check_10_input_mic() -> tuple[str, dict]:
    """Mic capture: synthesize speech with `say`, play it back, let the
    always-on mic listener capture it. Routing depends on what the engine
    selected as its input device:

      * Built-in / USB mic ("MacBook Air Microphone", "the printer123",
        etc.): use `afplay` so the audio comes out of the laptop
        speakers and the room mic picks it up acoustically.
      * BlackHole 2ch (virtual loopback): the engine's "mic" is a
        software input that pairs with BlackHole's output, so play the
        AIFF directly into BlackHole's output via sounddevice. This is
        still the same engine code path (the proc loop reads the
        InputStream callback and runs parakeet ASR), but the audio
        moves through the kernel ext rather than the air.

    The artifact records the chosen path so the proof is honest about
    which modality was exercised on this machine.
    """
    ...
    try:
        start_resp = json.loads(_post(f"{ENGINE_URL}/api/listen/start", {},
                                       timeout=20).read())
    except Exception:
        start_resp = {}
    _reset_listen()
    ...
    device_row = start_resp.get("audio_device") or {}
    device_name = str(device_row.get("name") or "").lower()
    is_blackhole = "blackhole" in device_name or device_row.get(
        "kind") == "virtual"

    modality = ""
    play_error = ""

    def _play_blackhole(target_device: int | None) -> None:
        import soundfile as sf
        import sounddevice as sd
        import numpy as np

        data, sr = sf.read(str(aiff))
        # Pad with short silence so the audio doesn't bleed into a
        # window boundary, and normalize amplitude — at very low RMS
        # parakeet-mlx tends to truncate the tail and emit fragments
        # like "I should get the old.". Normalizing to 0.95 peak makes
        # the ASR consistent without distortion.
        if data.ndim > 1:
            data = data.mean(axis=1)
        pad = np.zeros(int(sr * 0.5), dtype=data.dtype)
        data = np.concatenate([pad, data, pad])
        peak = float(np.max(np.abs(data)) or 1.0)
        if peak > 0:
            data = (data / peak) * 0.95
        sd.play(data.astype("float32"), sr, device=target_device)
        sd.wait()
        ...
        try:
            ...
            for idx, dev in enumerate(sd.query_devices()):
                if (isinstance(dev, dict)
                        and "blackhole" in str(dev.get("name", "")).lower()
                        and int(dev.get("max_output_channels") or 0) > 0):
                    target_device = idx
                    break
            _play_blackhole(target_device)
            modality = (f"sounddevice -> BlackHole 2ch (output device "
                        f"index {target_device}); engine InputStream on "
                        f"{device_row.get('name')}")
        except Exception as e:
            play_error = f"{type(e).__name__}: {e}"
            modality = f"blackhole_route_failed: {play_error}"
    else:
        # Acoustic speaker-to-mic path. afplay routes to the default
        # output device, the engine's selected input picks it up.
        try:
            subprocess.run(["afplay", str(aiff)], capture_output=True,
                           timeout=120)
            modality = ("acoustic speaker-to-mic via afplay; engine "
                        f"InputStream on {device_row.get('name')}")
        except Exception as e:
            play_error = f"{type(e).__name__}: {e}"
            modality = f"afplay_failed: {play_error}"
    # Retry the play a couple of times if the first window doesn't
    # land an act plan. The engine's window is 15s, so we re-play every
    # ~25s to give parakeet a fresh window to process clean audio.
    deadline = time.monotonic() + 180
    pending = None
    last_status = {}
    next_replay_at = time.monotonic() + 25.0
    replays = 0
    while time.monotonic() < deadline:
        ...
        if (plan.get("mode") or "").lower() == "act":
            break
        if time.monotonic() >= next_replay_at and replays < 3:
            try:
                if is_blackhole:
                    _play_blackhole(target_device)
                else:
                    subprocess.run(["afplay", str(aiff)],
...
def check_18_cleanup_passes() -> tuple[str, dict]:
    ...
    # Stop any running engine, then start fresh. SIGTERM first, then SIGKILL
    # for any pid that doesn't die in 4s. uvicorn under load needs the harder
    # signal; otherwise the port stays bound and the new engine cannot take
    # over, so the test ends up querying the OLD engine and sees the prior
    # onboarded=true state instead of fresh state.
    def _kill_port_8731():
        try:
            ...
            return []
        return [int(p) for p in pid_lines if p.strip()]

    for pid in _kill_port_8731():
        try:
            os.kill(pid, 15)
        except Exception:
            pass
    # Wait up to 4s for SIGTERM to take effect.
    deadline = time.monotonic() + 4
    while time.monotonic() < deadline:
        if not _kill_port_8731():
            break
        time.sleep(0.3)
    ...
    for pid in _kill_port_8731():
        try:
            ...
            pass
    # Wait up to 6s for the OS to fully release the port.
