...
def test_subtask_completes_when_completion_says_done():
    # completion call returns done=true immediately.
    c = _client('{"done":true,"evidence":"answer visible","answer":"1991"}')
    r = DSv4SkillRunner(client=c, verifier=MagicMock())
    sess = MagicMock()
    ...
    assert out["answer"] == "1991"


def test_subtask_dispatches_then_completes():
    # iter0: not done -> decide click -> verifier CERTIFIED
    # iter1: completion done
    c = MagicMock()
    c.chat.side_effect = [
        ORResponse(content='{"done":false,"evidence":"not yet"}', model="m", latency_s=0.1),
        ORResponse(content='{"action":"click","target_ref":"@e1"}', model="m", latency_s=0.1),
        ORResponse(content='{"done":true,"evidence":"done now","answer":"ok"}', model="m", latency_s=0.1),
    ]
    verifier = MagicMock()
...
def test_diverged_twice_escalates_model():
    # completion never done; decide returns click; verifier DIVERGED.
    def chat_side(*a, **kw):
        body = a[0][0]["content"] if a else kw["messages"][0]["content"]
        if "Done?" in body:
            return ORResponse(content='{"done":false,"evidence":"no"}', model="m", latency_s=0.1)
        return ORResponse(content='{"action":"click","target_ref":"@e1"}', model="m", latency_s=0.1)
    c = MagicMock()
    c.chat.side_effect = chat_side
    verifier = MagicMock()
    verifier.verify.return_value = Verdict("DIVERGED", "nothing changed", 0.9)
    r = DSv4SkillRunner(client=c, verifier=verifier, max_iters=6)
    sess = MagicMock()
...
def test_no_confirmation_gate_on_send():
    ...
    c = MagicMock()
    c.chat.side_effect = [
        ORResponse(content='{"done":false,"evidence":"x"}', model="m", latency_s=0.1),
        ORResponse(content='{"action":"click","target_ref":"@e9"}', model="m", latency_s=0.1),
        ORResponse(content='{"done":true,"evidence":"sent","answer":"sent"}', model="m", latency_s=0.1),
    ]
    verifier = MagicMock()
