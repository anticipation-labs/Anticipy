...
def _asr_thread_initializer() -> None:
    global _asr_thread_id
    _asr_thread_id = threading.get_ident()
    try:
        ...
        # The CPU fallback and older MLX builds still work without explicit
        # stream setup. Fail closed in _asr_tokens_impl if transcription errors.
        pass
...
def _get_asr_executor() -> ThreadPoolExecutor:
    ...
    if _asr_executor is None:
        # FIX (W2O): the MLX parakeet path was hard-coded to one worker
        # which serialized every concurrent audio upload (12s audio ->
        # 12s ASR, second upload waits 12s before starting). The thread
        # initializer already calls `mx.new_thread_local_stream(mx.gpu)`,
        # so multi-thread was always supported, just never exercised.
        # Default to 2; env-gated via ANTICIPY_ASR_WORKERS so a single
        # large parakeet weights load can be opted back to one worker
        # if memory pressure ever shows up.
        try:
            _w = int(os.environ.get("ANTICIPY_ASR_WORKERS", "2") or "2")
        ...
        _asr_executor = ThreadPoolExecutor(
            max_workers=_w,
            thread_name_prefix="anticipy-asr-mlx",
            initializer=_asr_thread_initializer,
        )
...
def _log_asr_exception(e: Exception, wav: np.ndarray) -> None:
    try:
        import json
        import time
        import traceback
        ...
        log.open("a", encoding="utf-8").write(json.dumps({
            ...
            "samples": int(len(wav)) if wav is not None else 0,
            "rms": float(np.sqrt(np.mean(np.asarray(wav) ** 2)) or 0.0)
                   if wav is not None and len(wav) else 0.0,
        }) + "\n")
    except Exception:
        pass


def _asr_tokens_impl(wav: np.ndarray) -> AsrResult:
...
def asr_tokens(wav: np.ndarray) -> AsrResult:
    """Thread-stable ASR entrypoint.

    FastAPI sync handlers and the listen worker can call this from different
    threads. MLX streams are thread-local, so Parakeet must be loaded and run on
    one dedicated worker thread instead of whichever thread received the HTTP
    request.
    """
    if threading.get_ident() == _asr_thread_id:
        return _asr_tokens_impl(wav)
    try:
        return _get_asr_executor().submit(_asr_tokens_impl, wav).result()
    except Exception as e:
        _log_asr_exception(e, wav)
        return AsrResult(text="", tokens=[])
