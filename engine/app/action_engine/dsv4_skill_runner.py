...
_DECIDE_SYS = (
    ...
    "fresh sign-in when an existing session is offered.\n"
    "SLACK specifically: 'Open' and 'LAUNCH SLACK' buttons are "
    "slack:// desktop-app launchers and do NOTHING in a browser. The "
    "ONLY reliable way into the Slack web client is to NAVIGATE "
    "directly to the workspace subdomain URL: https://<workspace>"
    ".slack.com (the workspace name/domain is visible on the signin "
    "or 'Welcome back' page, e.g. anticipy.slack.com). The session is "
    "shared so this loads that workspace's web client and its "
    "channels/messages. Do NOT click Open/Launch buttons and do NOT "
    "use app.slack.com root (that is only a workspace chooser).\n"
    ...
    "5) DRAFT ONLY: do NOT click Send. Save as draft instead by "
    "pressing Cmd+S (key 'meta+s'). The draft appears in the user's "
    "Drafts folder. When the 'Draft saved' indicator shows or the "
    "compose window closes after save, action done. NEVER click any "
    "button named 'Send' or 'Send (Cmd+Enter)' or use Cmd+Enter. "
    "Real-send is gated by ANTICIPY_ALLOW_REAL_SEND=1 env var, which "
    "is OFF by default in dev/test to prevent emails accidentally "
    "going to real people in the user's contacts. If you must send "
    "because the env explicitly enabled it AND the recipient is "
    "the user's own email (omarkebrahim@gmail.com) OR a test "
    "address (matches *+anticipy-*@gmail.com), THEN you may click "
    "Send. Otherwise: draft only.\n"
