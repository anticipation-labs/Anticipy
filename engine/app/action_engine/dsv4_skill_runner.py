...
_DECIDE_SYS = (
    ...
    "5) Send by CLICKING the Send button: it is an accessibility-tree "
    "button named exactly 'Send' (sometimes 'Send (Cmd+Enter)') at "
    "the bottom-left of the compose window. Use its @eN target_ref. "
    "Do NOT use a keyboard shortcut to send (synthetic Ctrl/Cmd+Enter "
    "is unreliable). Do NOT reopen Compose or re-type fields. After "
    "the click, if the compose window closed or 'Message sent' shows, "
    "action done. The post-action notify_user step (in the dispatcher) "
    "will text + email the user a receipt so they know what Anticipy "
