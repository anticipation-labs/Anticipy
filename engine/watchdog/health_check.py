...
Verifies:
  - Anticipy native bridge :7777 responds to /status
...
def probe_native_bridge(port: int = 7777) -> CheckResult:
    start = time.monotonic()
    try:
        r = httpx.get(f"http://127.0.0.1:{port}/status", timeout=3.0)
        elapsed = int((time.monotonic() - start) * 1000)
        if r.status_code == 200:
            body = r.json()
            detail = f"bridge_closed={body.get('bridge_closed')}"
            return CheckResult("native_bridge_7777", True, elapsed, detail)
        return CheckResult("native_bridge_7777", False, elapsed, f"http={r.status_code}")
    except Exception as e:
        elapsed = int((time.monotonic() - start) * 1000)
        return CheckResult("native_bridge_7777", False, elapsed, f"error={e}")
...
class HealthCheck:
    ...
    def run(self) -> HealthCheckRun:
        ...
        run.checks.append(probe_native_bridge())
