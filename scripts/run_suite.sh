...
for t in bus workers gateway gateway_retry orchestrator glassbox_rotation proactive triage triage_clause_scope harmline storesite trigger trigger_notify trigger_persistence follow_up_fires duetime decider decision_wall deferred_persistence pending_persistence ask_roundtrip ask_debounce annoyance interrupt_cap digest frontend_api glassbox_scorecard api_hand api_readback api_calendar_window token_vault api_vault core_api_mesh browser_hand browser_prompt_injection purchase_guard browser_safety_loop browser_use_cdp browser_available_probe browser_result_on_card navwall form_prepare handoff site_hints agent_proof channels channel_worker conversation_relay inbound agent_reply voice memory memory_capture memory_inject memory_maintain memory_infer memory_selfcheck memory_glue memory_remembered review_infer press_go dryrun_preview demo_day readiness mac_mic inputs_same_brain onboarding_profile onboarding_profile_endpoint onboarding_scan onboard_discover onboard_scan clarify owner_mode owner_onboarding owner_ingest_event owner_upload_ingest owner_api_auth endpoint_hardening public_backend_path messy_proactive_handoff owner_duplicate_collapse memory_handoff memory_intent_gate autonomy_modes_gate onboarding_frontdoor preview_moat_rescue browser_binary_selfheal directed_question_aside twentylife_floor_fixes; do
