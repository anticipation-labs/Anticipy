...
for t in bus workers gateway gateway_retry orchestrator proactive triage triage_clause_scope harmline storesite trigger trigger_notify trigger_persistence duetime decider decision_wall deferred_persistence pending_persistence ask_roundtrip ask_debounce annoyance interrupt_cap frontend_api glassbox_scorecard api_hand api_readback token_vault api_vault browser_hand browser_prompt_injection purchase_guard browser_safety_loop navwall form_prepare handoff site_hints agent_proof channels channel_worker conversation_relay inbound memory memory_capture memory_inject memory_maintain memory_infer memory_selfcheck memory_glue memory_remembered review_infer press_go dryrun_preview demo_day readiness onboarding_profile onboarding_profile_endpoint clarify owner_mode owner_onboarding owner_ingest_event owner_upload_ingest owner_api_auth endpoint_hardening public_backend_path messy_proactive_handoff; do
