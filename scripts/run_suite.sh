...
run onboarding_firstcards_e2e_selftest "$PY" "$REPO/engine/scripts/test_onboarding_firstcards_e2e.py" --selftest  # ONBOARDING PAYOFF: a THROWAWAY engine (own ephemeral port, not :8787) drives land->connect->INHALE->complete over real HTTP and asserts REAL FIRST CARDS derived from the inhaled account land on the board (drawers) — people/site tokens that exist ONLY in the inhale — + planted battery (corrupt inhale: walled / empty -> NO fake cards)
