...
Latest builder lap: `20260608T064322Z` is `PENDING_JUDGE`, not proof. Production-linked repo `/Users/omarebrahim/Developer/Anticipy-DEV-FINAL` branch `rebuild/spine-clean` now has tracked product commit `cf8178e2c2454fe91a8b86788656d206d23eab5a`. Explicit typed, fully time-grounded Calendar tasks now attempt a Google Calendar API insert on the primary calendar before falling back to the Google Calendar browser template path. The new helper uses the existing encrypted OAuth token, refreshes through Google's OAuth token endpoint when needed, creates with `sendUpdates=none`, returns `event_id` and `html_link` proof, and does not log token material. The sidecar build script includes the helper as a PyInstaller hidden import. Checks passed: official Google docs review, Python compile, `git diff --check`, staged diff check, forbidden-literal scan with no matches, fake-network Calendar insert probe, mocked server branch probe verifying `SUCCESS` plus pending-state cleanup, `bash scripts/build_dmg.sh` before and after product commit, recursive sidecar archive helper check, embedded commit verification, strict codesign, and Computer Use read-only build-path app inspection showing the real Anticipy window with the task box and browser warning. Final local DMG is `178873974` bytes with SHA-256 `8e9611723bf91cd1959116067f1b852c91c853b142aba31afbfb102e03b49754`. No real Calendar event was created by the builder. The separate judge has not verified it.
...
- Treating production-linked source commits `235dc1f3`, `0c867f96`, `049f4ad0`, `47e92b6e`, `b57e3b1a`, `71829156`, `5942dc0d`, or `cf8178e2` as M2 or M3 proof. They are pending until the separate judge verifies the packaged app, typed task, API artifact, or browser hands on a safe, reversible task.
...
- While judge quota is blocked, keep improving unblocked production-source perimeter slices without claiming proof. The next useful slice is exposing clearer API-backed Calendar proof/read-back status in the typed-task UI or continuing safe browser-hands readiness work.
