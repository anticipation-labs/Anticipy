...
// The DMG lives in Cloudflare R2 (anticipy-downloads bucket). Vercel reads
// NEXT_PUBLIC_R2_PUBLIC_URL from .env.local and inlines it into the client
// bundle at build time. If it is unset we fall back to the legacy /dl/ path
// so local dev and the F-004 audit do not silently 404.
const DMG_FILENAME = "Anticipy_1.0.0_aarch64.dmg";
const R2_BASE = (process.env.NEXT_PUBLIC_R2_PUBLIC_URL ?? "").replace(/\/+$/, "");
const DMG_URL = R2_BASE ? `${R2_BASE}/${DMG_FILENAME}` : `/dl/${DMG_FILENAME}`;
