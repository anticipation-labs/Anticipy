...
class NativeBridge:
    ...
    async def click(self, selector: str) -> dict:
        result = await self._send_and_await({"type": "click", "selector": selector})
        data = result.get("data")
        return data if isinstance(data, dict) else {}

    async def type(self, selector: str, text: str, *, submit: bool = False) -> dict:
        result = await self._send_and_await({
            "type": "type", "selector": selector, "text": text, "submit": bool(submit),
        })
        data = result.get("data")
        return data if isinstance(data, dict) else {}
    ...
    async def get_url(self) -> str:
        result = await self._send_and_await({"type": "getDOMSnapshot", "limit": 0})
        data = result.get("data")
        if isinstance(data, dict):
            u = data.get("url")
            if isinstance(u, str):
