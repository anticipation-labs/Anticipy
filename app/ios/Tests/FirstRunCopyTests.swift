...
// quietly testing a shape the app no longer has. Seven since build 139, when
// the Connections spec's step 2 — "which apps do you live in?" — went in
// between the pendant's offer and the microphone beat. It had been written,
// compiled into the target and given zero call sites.
let pages = 7
...
check("the track counts eight beats, including the account", FirstRunTrack.count == 8)
...
// six-element array is the shape of this fix going half-done: the numbers
// would move while every beat wore the name of the one before it.
...
check("VoiceOver hears the same count the screen prints",
      FirstRunTrack.spokenLabel(step: 1, pageCount: pages) == "Step 3 of 8, How I work")
