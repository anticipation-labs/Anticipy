...
static bool boot_gesture_consumed;
static bool provisioning_requested;
static bool factory_reset_requested;

#define OWNER_PROVISION_HOLD_MS 3000
#define OWNER_FACTORY_RESET_HOLD_MS 10000
#define OWNER_GESTURE_POLL_MS 25

static void sample_owner_boot_gesture(void)
{
#ifndef CONFIG_BT_FILTER_ACCEPT_LIST
    return;
#else
    if (gpio_pin_get_dt(&button_pin) <= 0) {
        return;
    }

    const int64_t started = k_uptime_get();
    int64_t held_ms = 0;
    bool threshold_indicated = false;
    LOG_INF("D7 boot hold detected; release after 3 s to provision or hold 10 s to reset owner");

    while (gpio_pin_get_dt(&button_pin) > 0) {
        held_ms = k_uptime_get() - started;
        if (!threshold_indicated && held_ms >= OWNER_PROVISION_HOLD_MS) {
            set_led_blue(true);
            threshold_indicated = true;
        }
        if (held_ms >= OWNER_FACTORY_RESET_HOLD_MS) {
            break;
        }
        k_msleep(OWNER_GESTURE_POLL_MS);
    }
    ...
    if (held_ms >= OWNER_FACTORY_RESET_HOLD_MS) {
        ...
        boot_gesture_consumed = true;
        set_led_red(true);
        k_msleep(250);
        set_led_red(false);
        LOG_WRN("D7 owner factory-reset gesture accepted");
    } else if (held_ms >= OWNER_PROVISION_HOLD_MS) {
        provisioning_requested = true;
        boot_gesture_consumed = true;
        LOG_WRN("D7 owner provisioning gesture accepted");
    }
#endif
}
...
static inline void notify_press()
{
    ...
    if (conn != NULL) {
        if (transport_peer_is_authorized(conn)) {
            bt_gatt_notify(conn, &button_service.attrs[2], &final_button_state, sizeof(final_button_state));
        }
        bt_conn_unref(conn);
    }
}
...
int button_init()
{
    ...
    /* This is sampled before interrupts and the runtime long-press state
     * machine start, so a normal in-use three-second press can never erase an
     * owner.  GPIO_ACTIVE_LOW makes gpio_pin_get_dt() return logical pressed. */
    sample_owner_boot_gesture();
    ...
    was_pressed = !boot_gesture_consumed && gpio_pin_get_dt(&button_pin) > 0;
...
{
    bool requested = provisioning_requested;
...
{
    bool requested = factory_reset_requested;
